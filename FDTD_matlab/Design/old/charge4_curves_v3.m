...
x=0:nr;
y=sin(acos((2.^(x-1)-1)./(2.^(x-1)))).*(2.^(x-1));
F2 = Lb_max*F_max/2;
disk(0,r_max,F2)
disk(0,-r_max,F2)
xV = F2;
yV = r_max;
patch([xV xV -xV -xV],[yV -yV -yV yV],mygrey,'edgecolor',mygrey)
disk(Lb_max,0,F2)
disk(-Lb_max,0,F2)
%count = nc
...
for ii=1:nr
    ...
    disk(-xii,yii,F2)
    disk(-xii,-yii,F2)
    ang = atan2(rii-xii,yii);
    nbii=ceil(2*(pi-ang)*rii/Lb_max*nb_pts+1);
    thii=linspace(ang,2*pi-ang,nbii);
    [x,y] = pol2cart(thii,rii-F2);
    [x2,y2] = pol2cart(thii,rii+F2);
    x(end+1:end+nbii) = x2;
    y(end+1:end+nbii) = -y2;
	patch(rii-x,y,mygrey,'edgecolor',mygrey);
	patch(-rii+x,y,mygrey,'edgecolor',mygrey);
end
% for ii=0:nr-2
%     rtemp = 2^ii*3/2;
%     disk(Lb_max*rtemp,Lb_max*rtemp,F2)
%     disk(Lb_max*rtemp,-Lb_max*rtemp,F2)
%     disk(-Lb_max*rtemp,Lb_max*rtemp,F2)
%     disk(-Lb_max*rtemp,-Lb_max*rtemp,F2)
% end
