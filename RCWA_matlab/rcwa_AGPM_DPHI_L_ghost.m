...
global dphi_sp_T nul_res_sp_b null_res_sp retard n1 n2 n3 Fnew dnew pente fld1 n2lb limZOG prof ghost
...
ylabel('Phase shift (rad)','FontSize',fsz*1.2)
y1=plot(lb_t,dphi_tmp1,'b','linewidth',lwz);
y2=plot(lb_t,dphi_gst1,'b:','linewidth',lwz);
y3=plot(lb_t,dphi_tmp2,'r','linewidth',lwz);
y4=plot(lb_t,dphi_gst2,'r:','linewidth',lwz);

%c=plot(lb_t,lb_t.*0+mean(nul_res_sp_b),'k-.','linewidth',lwz)
%l=legend(' Null Depth (AGPM3)',' Mean null depth');

l=legend(' AGPM-L1: F=0.36 h=4.2µm',' AGPM-L1 ghost',' AGPM-L2: F=0.36 h=3.8µm',' AGPM-L2 ghost');
...
t=title(['     Phase shift for \alpha = ',sprintf('%3.2f',pente_deg),'°']);%    with B = [',sprintf('%3.1f',lb_min),' - ',sprintf('%3.1f',lb_max),' µm] and \Lambda = ',sprintf('%3.2f',Lb),' µm']);
set(t,'Fontname',fnz,'FontSize',fsz*1.2,'FontWeight',fwz,'HorizontalAlignment','center')

print('-dpng', ['lognuldpt_L+ghost.png'], '-r300');
