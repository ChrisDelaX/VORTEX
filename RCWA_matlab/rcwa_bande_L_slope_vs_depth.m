...
L=16;%20;%
pente=rad(4.3);
pente_min=rad(4);pente_max=rad(4.6);
...
% --> limite Diamant bande N : 3.78  L : 1.4697
Lb=1.4697;
...
for i=1:npts
    ...
    i
    %[x,fval,exitflag] = fminsearch(@null,x0,optimset('MaxIter',15,'Display','iter','TolX',1e-2,'TolFun',1e-3));
    [x,fval,exitflag] = fminsearch(@null_opt2,x0,optimset('MaxIter',15,'Display','iter','TolX',1e-6,'TolFun',1e-6));
    %fval=null([]);
...
hold on
nulldepth = subplot(1,4,1:2);
%title('AGPM Diamond/SiO2 : K-band')
plot(Xparam,nulldpt,'Marker','.')
xlabel('Profile slope \alpha (°)')
%xlabel('Depth (\mum)')
title('Null Depth @ 3.85 \mum')
axis square
set(gca,'YScale','log')
...
% xlabel('Depth (\mum)')
% ylabel('Period (\mum)')
ParamY2 = subplot(1,4,3);
plot(Xparam,Yparam(:,1),'Marker','.')
xlabel('Profile slope \alpha (°)')
%xlabel('Depth (\mum)')
title('Depth (\mum)')
AspRatio = subplot(1,4,4);
% for i=1:npts
%     if Yparam(i) < 0.5
%         F_AR(i) = Yparam(i);
%     else
