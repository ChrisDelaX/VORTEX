...
Trans_tmp1 = (tmp1+tmp2)./2;%.*100
TransL1 = sum(Trans_tmp1.*p_wide(lb_t)')/sum(p_wide(lb_t))
TransL1_filter = sum(Trans_tmp1.*p_wide(lb_t)'.*p_wide(lb_t)')/sum(p_wide(lb_t))
...
Trans_tmp2 = (tmp1+tmp2)./2;%.*100
TransL2 = sum(Trans_tmp2.*p_wide(lb_t)')/sum(p_wide(lb_t))
TransL2_filter = sum(Trans_tmp2.*p_wide(lb_t)'.*p_wide(lb_t)')/sum(p_wide(lb_t))
...
Trans_tmp3 = (tmp1+tmp2)./2;%.*100
TransL3 = sum(Trans_tmp3.*p_wide(lb_t)')/sum(p_wide(lb_t))
TransL3_filter = sum(Trans_tmp3.*p_wide(lb_t)'.*p_wide(lb_t)')/sum(p_wide(lb_t))
...
y1=plot(lb_t,Trans_tmp1,'r','linewidth',lwz);
y2=plot(lb_t,Trans_tmp2,'g','linewidth',lwz);
y3=plot(lb_t,Trans_tmp3,'b','linewidth',lwz);
y4=plot(lb_t,Trans_tmp4,'k','linewidth',lwz);
y5=plot(lb_t,TransBS,'k--','linewidth',lwz);

legendL1=sprintf(' AGPM-L1: Trans=%3.2f',TransL1);
legendL2=sprintf(' AGPM-L2: Trans=%3.2f',TransL2);
legendL3=sprintf(' AGPM-L3: Trans=%3.2f',TransL3);
legendL4=sprintf(' AGPM-L4: Trans=%3.2f',TransL4);
l=legend(legendL1,legendL2,legendL3,legendL4,' without SWG');
...
% %grid on
% set(gca,'box','on','linewidth',2)
% set(gca,'XLim',[3.4 4.1],'xtick',bandtick)
% %set(gca,'YScale','log')
% set(gca,'ylim',[0.65 0.93])%,'ytick',[.4 .5 .6 .7 .8 .9 1])
% set(gca,'Fontname',fnz,'FontSize',fsz,'FontWeight',fwz)
% set(gca,'XMinorTick','on')
% set(gca,'YMinorTick','on')
% xlabel('Wavelength (\mum)','FontSize',fsz*1.2)
% ylabel('Transmission','FontSize',fsz*1.2)
% y1=plot(lb_t,Trans_tmp1.*p_wide(lb_t)','r','linewidth',lwz);
% y2=plot(lb_t,Trans_tmp2.*p_wide(lb_t)','g','linewidth',lwz);
% y3=plot(lb_t,Trans_tmp3.*p_wide(lb_t)','b','linewidth',lwz);
% y4=plot(lb_t,Trans_tmp4.*p_wide(lb_t)','k','linewidth',lwz);
% y5=plot(lb_t,p_wide(lb_t)','k--','linewidth',lwz);
% 
% legendL1=sprintf(' AGPM-L1: Trans_{filter}=%3.2f%',TransL1_filter);
% legendL2=sprintf(' AGPM-L2: Trans_{filter}=%3.2f%',TransL2_filter);
% legendL3=sprintf(' AGPM-L3: Trans_{filter}=%3.2f%',TransL3_filter);
% legendL4=sprintf(' AGPM-L4: Trans_{filter}=%3.2f%',TransL4_filter);
% l=legend(legendL1,legendL2,legendL3,legendL4,' Filter WIDE');
% %l=legend(' Optim1: F=0.403 h=4.303µm',' Optim2: F=0.4 h=4.273µm',' AGPM-L1: F=0.36 h=4.2µm',' AGPM-L2: F=0.36 h=3.8µm');
% set(l,'Fontname',fnz,'FontSize',fsz*0.9,'location','best')
% set(l,'box','on','linewidth',2)
% 
...
% set(t,'Fontname',fnz,'FontSize',fsz*1.2,'FontWeight',fwz,'HorizontalAlignment','center')
% 
% print('-dpng', [sprintf('transmission_4L_filter.png')], '-r300');
...
set(gca,'XLim',band,'xtick',bandtick)
set(gca,'YScale','log')
set(gca,'ylim',[.0000001 100])%,'ytick',[.4 .5 .6 .7 .8 .9 1])
set(gca,'Fontname',fnz,'FontSize',fsz,'FontWeight',fwz)
set(gca,'XMinorTick','on')
set(gca,'YMinorTick','on')
xlabel('Wavelength (\mum)','FontSize',fsz*1.2)
...
y5=plot(lb_t,nul_tmp1*0.00122,'r--','linewidth',lwz);
y6=plot(lb_t,nul_tmp2*0.00073,'g--','linewidth',lwz);
y7=plot(lb_t,nul_tmp3*0.00111,'b--','linewidth',lwz);
y8=plot(lb_t,nul_tmp4*0.00121,'k--','linewidth',lwz);
...
print('-dpng', [sprintf('rejection_4L_ghost.png')], '-r300');
