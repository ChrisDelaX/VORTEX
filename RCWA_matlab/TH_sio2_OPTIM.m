...
lb=2.2;%
lb0=lb;
lb_min=2;%.53;%.65;%1.15;%1.5;%
lb_max=2.4;%.65;%.79;%1.4;%1.8;%
nlb=11;%21;%81;%
...
% leg=legend(' \lambda = 1.5-1.8 \mum (H band)',' \pi-phase shift');%,'location','northeast');
...
% %' \lambda = 0.53-0.65 \mum (visible)'
% %' \lambda = 0.65-0.78 \mum (visible)'
% %' \lambda = 1.15-1.4 \mum (J band)'
% %' \lambda = 1.5-1.8 \mum (H band)'
% %' \lambda = 2-2.4 \mum (K band)'
% 
% 
% break
% break
...
% xlabel('Period  \Lambda (\mum)','FontSize',fsz)
% ylabel(AX(1),'Filling factor  F','FontSize',fsz)
% ylabel(AX(2),'Grating depth  h (\mum)','FontSize',fsz)
