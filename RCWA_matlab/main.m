...
Tuu_l=eye(2*(2*N+1));
Rud_l=sparse(2*(2*N+1),2*(2*N+1));
Rdu_l=sparse(2*(2*N+1),2*(2*N+1));
Tdd_l=eye(2*(2*N+1));

%Calcul de W_lp1 initial
%-----------------------

F_lp1=[eye(2*N+1)                         sparse(2*N+1,2*N+1)                       eye(2*N+1)                       sparse(2*N+1,2*N+1)                  
    sparse(2*N+1,2*N+1)                       eye(2*N+1)                         sparse(2*N+1,2*N+1)                     eye(2*N+1)
...
for l=1:L
    ...
    if var(prof(l,:))<1e-10 % si partout le meme indice sur la couche l
        prof_tmp=prof(l,1);
        clear E; E=eye(2*N+1)*prof_tmp;
        clear A; A=eye(2*N+1)/prof_tmp;
    ...
    s=[t11-t12*invt22*t21  t12*invt22
        -invt22*t21         invt22];
    clear t11 t12 t21 t22 invt22
    
    %Matrice s chapeau
    %-----------------
    
    s_chap_g=[eye(2*(2*N+1))    sparse(2*(2*N+1),2*(2*N+1)) 
        sparse(2*(2*N+1),2*(2*N+1))  X_l];   
    s_chap_d=[X_l               sparse(2*(2*N+1),2*(2*N+1)) 
        sparse(2*(2*N+1),2*(2*N+1))  eye(2*(2*N+1))];   
    clear X_l
    
    s_chap=s_chap_g*s*s_chap_d;
    clear s
...
s_chap_g=[eye(2*(2*N+1))    sparse(2*(2*N+1),2*(2*N+1)) 
    sparse(2*(2*N+1),2*(2*N+1))  X_lp1];   
s_chap_d=[X_lp1             sparse(2*(2*N+1),2*(2*N+1)) 
    sparse(2*(2*N+1),2*(2*N+1))  eye(2*(2*N+1))];   
clear X_lp1
...
phi_s_R=atan2(imag(Rs(N+1)),real(Rs(N+1)));
phi_p_T=atan2(imag(Tp(N+1)),real(Tp(N+1)));
phi_p_R=atan2(imag(Rp(N+1)),real(Rp(N+1)));

dphi_sp_T=phi_s_T-phi_p_T;
dphi_sp_R=phi_s_R-phi_p_R;
