...
nb_theta = nb_theta+1;
theta = linspace(0,pi/2,nb_theta)';
...
for j=1:4*nb_theta
    ...
    if abs(cos(theta(j)))>.1%.02%
        r = linspace(1/abs(cos(theta(j)))/(2*nb_r+1),1/abs(cos(theta(j))),nb_r);
        for i=1:nb_r*abs(cos(theta(j)))
            %quiver(r(i)*cos(theta(j)),r(i)*sin(theta(j)),-r(i)*sin(lp/2*theta(j)),r(i)*cos(lp/2*theta(j)),scale/abs(cos(theta(j)))^(1/2),'k.')
            %quiver(r(i)*cos(theta(j)),r(i)*sin(theta(j)),r(i)*sin(lp/2*theta(j)),-r(i)*cos(lp/2*theta(j)),scale/abs(cos(theta(j)))^(1/2),'k.')
            q1=quiver(r(i)*cos(theta(j)),r(i)*sin(theta(j)),-r(i)*sin(lp/2*theta(j)),r(i)*cos(lp/2*theta(j)),scale/abs(cos(theta(j))),'k.');
            q2=quiver(r(i)*cos(theta(j)),r(i)*sin(theta(j)),r(i)*sin(lp/2*theta(j)),-r(i)*cos(lp/2*theta(j)),scale/abs(cos(theta(j))),'k.');
            if i==1 && j==1
...
%save
print('-depsc2',sprintf('lines_v2b_th=%d_r=%d.eps',nb_theta-1,nb_r), '-r300');
