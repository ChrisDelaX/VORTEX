...
% 0.5329];
  ...
  %  for i=1:npts
   ...
   %     i
        [x,fval,exitflag] = fminsearch(@null_ordres,x0,optimset('MaxIter',15,'Display','iter','TolX',1e-2,'TolFun',1e-3));
        %[x,fval,exitflag] = fminsearch(@null,x0,optimset('MaxIter',35,'Display','iter','TolX',5e-3,'TolFun',1e-5));
        fval=null_ordres(x0);
        

        Xparam(i,1)=pente/pi*180;
        YparamTE(i,:)=tmp1(1,:);
        YparamTM(i,:)=tmp2(1,:);
        sommeT(i)=sumED_Ts(1);
        sommeTR(i)=sumED_Ts(1)+sumED_Rs(1);
        nulldpt(i)=fval;
   % end
            plot(Xparam,YparamTE)
        %plot(Xparam,sommeTR)
%end
toc
save rcwa_pente_Noptim.mat
break
...
plot(Xparam,sommeT,'k-','Linewidth',2)
%plot(Xparam,sommeTR,'k-','Linewidth',2)
plot(Xparam,YparamTE(:,1),'k--','Linewidth',2)
plot(Xparam,YparamTE(:,2),'k--','Linewidth',2)
plot(Xparam,YparamTE(:,Nsim-4),'g:','Linewidth',2)
plot(Xparam,YparamTE(:,Nsim-3),'g:','Linewidth',2)
plot(Xparam,YparamTE(:,Nsim-2),'g:','Linewidth',2)
plot(Xparam,YparamTE(:,Nsim-1),'b--','Linewidth',2)
plot(Xparam,YparamTE(:,Nsim),'b--','Linewidth',2)
plot(Xparam,YparamTE(:,Nsim+1),'b-','Linewidth',2)
